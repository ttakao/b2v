...
OCR前にページを表示し、上下左右のCropを数値または境界線ドラッグで調整します。
...
b2v本体のライセンスは、このリポジトリの `LICENSE` を参照してください。

Style-Bert-VITS2本体、音声モデル、LLMモデル、Tesseract、FFmpegその他の依存物には、それぞれ別のライセンス・利用条件があります。
